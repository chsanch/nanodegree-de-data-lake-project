...
def process_song_data(spark, input_data, output_data):
    ...
    songs_table.write.mode("overwrite").partitionBy("year", "artist_id").parquet(
        os.path.join(output_data, "parquet/songs")
    )
    ...
    artists_table.write.mode("overwrite").parquet(
        os.path.join(output_data, "parquet/artists")
    )
...
def process_log_data(spark, input_data, output_data):
    """
    process_long_data: 
    function to proces the log data
    args: 
        spark: Spark object session
        input_data: filepath where the data is stored
        output_data: filepath where to store the processed data
    ...
    users_table.write.mode("overwrite").parquet(
        os.path.join(output_data, "parquet/users")
    )
    ...
    time_table.write.mode("overwrite").partitionBy("year", "month").parquet(
        os.path.join(output_data, "parquet/time")
    )
    ...
    songplays_table = (
        df.join(song_df, df.song == song_df.title, "inner")
        .selectExpr(
            "timestamp as start_time",
            "userId as user_id",
            "level",
    ...
    songplays_table.write.mode("overwrite").partitionBy("year", "month").parquet(
        os.path.join(output_data, "parquet/songplays")
    )
...
def main():
    ...
    this is the main function, it will:
        - get or create a spark session
        - execute the process_song_data function to load and process related
          data
        - execute the process_log_data funtion to load and process related data
    """
